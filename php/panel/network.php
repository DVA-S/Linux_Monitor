...
if (base64_decode($token) !== "" && $get_value !== "" && base64_decode($token) == $get_value && $now-$datatime <= 600) {
    ...
    //利用数据行数判定登录
    $stmt = $con->prepare("select data_time,network_up,network_down from (select data_time,network_up,network_down from bysj.network where host_ip = ? order by data_time desc limit 0,20) AS network order by data_time;");
