...
memory(){
    if [ $lang = "zh_CN.UTF-8" ]
    then
	      mysql -uroot -pEsxi0000. -e "insert into bysj.memory (host_ip,mem_used,mem_free,swap_used,swap_free,data_time) values
	            (
...
#CPU
cpu(){
	mysql -uroot -pEsxi0000. -e "insert into bysj.cpu(host_ip,cpu_used,data_time)
	values(
	`echo \'$(ip a | grep inet | sed -n '3p' | awk -F ' ' '{print $2}' | awk -F '/' '{print $1}')\'`,
...
network(){
  ...
  then
      #rxkB/s下载
    	mysql -uroot -pEsxi0000. -e "insert into bysj.network(host_ip,network_name,network_up,network_down,data_time)
    	values(
    	`echo \'$(ip a | grep inet | sed -n '3p' | awk -F ' ' '{print $2}' | awk -F '/' '{print $1}')\'`,
...
disk(){
	mysql -uroot -pEsxi0000. -e "insert into bysj.disk (host_ip,disk_read,disk_write,disk_used,disk_free,data_time)
	values(
	`echo \'$(ip a | grep inet | sed -n '3p' | awk -F ' ' '{print $2}' | awk -F '/' '{print $1}')\'`,
